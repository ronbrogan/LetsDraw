...
namespace Foundation.Rendering.Cameras
{
    public class FpCamera : ICamera
    {
        ...
        private void UpdateView()
        {
            ...
            var qPitch = Quaternion.CreateFromAxisAngle(new Vector3(1, 0, 0), Pitch);
            var qYaw = Quaternion.CreateFromAxisAngle(new Vector3(0, 1, 0), Yaw);

            //For a FPS camera we can omit roll
            var orient = Quaternion.Normalize(qPitch * qYaw);
            var rotation = Matrix4x4.CreateFromQuaternion(orient);
            var translation = Matrix4x4.CreateTranslation(Position * -1);

            ViewMatrix = Matrix4x4.Multiply(translation, rotation);
        }
        ...
        public void KeyPressed(Key key, double deltaTime)
        {
            ...
            var iPitch = Quaternion.CreateFromAxisAngle(new Vector3(1, 0, 0), -Pitch);
        ...
        public void UpdateProjectionMatrix(int width, int height)
        {
            ...
            ProjectionMatrix = Matrix4x4.CreatePerspectiveFieldOfView(fov, ar, near1, far1);
        }
    }
}
